...
class TimeDependentAD:    
    def __init__(self, mesh, Vh, prior, misfit, simulation_times, wind_velocity, gls_stab):
        ...
        # CHANGE: For midpoint, we need both left and right matrices
        # Left matrix: M + (dt/2)*(N + stabilization) 
        # Right matrix: M - (dt/2)*(N + stabilization)
        # The stabilization is split evenly between L and R
        self.L = self.M + 0.5*dt*self.N + 0.5*stab  # CHANGE: dt/2 factor
        self.R = self.M - 0.5*dt*self.N - 0.5*stab  # CHANGE: New matrix for RHS
        self.Lt = self.M + 0.5*dt*self.Nt + 0.5*stab  # CHANGE: dt/2 factor
        self.Rt = self.M - 0.5*dt*self.Nt - 0.5*stab  # CHANGE: Transpose of R for adjoint
    ...
    def solveFwd(self, out, x):
        ...
        for t in self.simulation_times[1::]:
            # CHANGE: Use R matrix instead of M_stab for RHS
            # (M - dt/2*(N+stab)) * u^n
            self.R.mult(uold, rhs)  # CHANGE: Was M_stab.mult, now R.mult
    ...
    def solveAdj(self, out, x):
        ...
        # CHANGE: Adjoint for midpoint solves L^T p^n = R^T p^{n+1} - grad_state
        for t in self.simulation_times[::-1]:
            # Multiply previous adjoint (p^{n+1}) by R^T
            self.Rt.mult(pold, rhs)  # CHANGE: Was Mt_stab, now Rt
            
            # Subtract gradient at this time
    ...
    def evalGradientParameter(self,x, mg, misfit_only=False):
        ...
        # CHANGE: Gradient uses R^T * p0 (was Mt_stab)
        # The gradient at t=0 comes from the adjoint at first time step
        Rt_p0 = dl.Vector()
        self.M.init_vector(Rt_p0, 0)
        self.Rt.mult(p0, Rt_p0)  # CHANGE: Was Mt_stab, now Rt
        mg.axpy(-1., Rt_p0)
    ...
    def solveFwdIncremental(self, sol, rhs):
        ...
        for t in self.simulation_times[1::]:
            # CHANGE: For incremental forward, use R for the homogeneous part
            self.R.mult(uold, Muold)  # CHANGE: Was M_stab, now R
    ...
    def solveAdjIncremental(self, sol, rhs):
        ...
        for t in self.simulation_times[::-1]:
            # CHANGE: For incremental adjoint, use Rt for the homogeneous part
            self.Rt.mult(pold, Mpold)  # CHANGE: Was Mt_stab, now Rt
            rhs.retrieve(myrhs, t)
            Mpold.axpy(1., myrhs)
    ...
    def applyC(self, dm, out):
        ...
        # CHANGE: applyC maps parameter to first state - use R for midpoint
        self.R.mult(dm, myout)  # CHANGE: Was M_stab, now R
        myout *= -1.
    ...
    def applyCt(self, dp, out):
        ...
        dp.retrieve(dp0, t)
        dp0 *= -1.
        # CHANGE: applyCt maps first adjoint to parameter - use Rt
        self.Rt.mult(dp0, out)  # CHANGE: Was Mt_stab, now Rt
