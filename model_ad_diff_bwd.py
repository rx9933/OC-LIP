...
class TimeDependentAD:    
    def __init__(self, mesh, Vh, prior, misfit, simulation_times, wind_velocity, gls_stab):
        ...
        # Use the measure directly in the forms
        self.M = dl.assemble( ufl.inner(u, v) * ufl.dx(domain=mesh) )
        self.M_stab = dl.assemble( ufl.inner(u, v+tau*r_test) * ufl.dx(domain=mesh) )
        self.Mt_stab = dl.assemble( ufl.inner(u+tau*r_trial, v) * ufl.dx(domain=mesh) )
        
        Nvarf  = (ufl.inner(kappa * ufl.grad(u), ufl.grad(v)) + ufl.inner(wind_velocity, ufl.grad(u)) * v ) * ufl.dx(domain=mesh)
        Ntvarf  = (ufl.inner(kappa * ufl.grad(v), ufl.grad(u)) + ufl.inner(wind_velocity, ufl.grad(v)) * u ) * ufl.dx(domain=mesh)
        self.N  = dl.assemble( Nvarf )
        self.Nt = dl.assemble( Ntvarf )
        
        stab = dl.assemble( tau * ufl.inner(r_trial, r_test) * ufl.dx(domain=mesh) )
        self.L = self.M + dt * self.N + stab
        self.Lt = self.M + dt * self.Nt + stab
