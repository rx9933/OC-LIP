...
class PathNetwork(torch.nn.Module):
    ...
    def forward(self, q, expected=None):
        c0 = q[:, :2]
        fourier_coeffs = self.base_model(q, expected=expected[:, 2:] if expected is not None else None)  # Predict only Fourier coefficients
...
m_data = mq_data_dict['m'] # fourier coefficients =   14 (mean position + 4*3modes of coefficients) for path
...
lr_scheduler = None# ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=20)
def make_combined_loss_with_obstacles(observation_times, K, omegas, buildings,
                                      fourier_weight=1.0, path_weight=1.0,
                                      boundary_weight=0.1, obstacle_weight=1000.0):
    def combined_loss_wrapper(pred, target):
        return combined_loss_with_components(
...
# Use it:
combined_loss_func = make_combined_loss_with_obstacles(
    observation_times, K, omegas, BUILDINGS,
    fourier_weight=100,      # Keep as is # x10
    path_weight=10,         # Keep as is
    boundary_weight=0.1,     # Small for domain boundary
    obstacle_weight=100.0  # HIGH for buildings (start with 10000)
)

# Train with separate loss tracking
network, history = train_with_separate_losses(
...
# Add to suptitle (add this before plt.tight_layout()):
...
plot_save_name = f"test_samples_{args.data_type}_ntrain{args.n_train}_percent_diff{percent_diff_avg:.2f}.png"
